...
unsigned long global; //for debugging
...
void SysTick_Handler(void)
{
  ...
  for (int i = 1; i < MAX_TASKS; i++) {
	  ...
	  // TASK IS DONE SLEEPING -> READY TO RUN
	  if (scheduler_TCB[i].sleep_count == 0  && scheduler_TCB[i].timestamp > 0 && scheduler_TCB[i].state != DORMANT){
	  ...
	  //TIMESTAMP HAS EXPIRED -> RESET
	  else if (scheduler_TCB[i].timestamp == 0 && scheduler_TCB[i].state != DORMANT) {
		  scheduler_TCB[i].state = READY;
		  scheduler_TCB[i].timestamp = scheduler_TCB[i].deadline;
		  yield_required = 1;
	  }
  ...
  if (yield_required == 1) {
	  ...
	  if(next_task != 0 && scheduler_TCB[0].state == RUNNING ){
